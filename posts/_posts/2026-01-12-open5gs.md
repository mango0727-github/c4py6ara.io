...
On the figure, you can see gNodeB at `127.0.0.200` first talks to the AMF at `127.0.0.5` with `INIT` and following communications, finally exchanging NGAP packets. NGAP packets are not showing up afterward because there is no UEs running in the cell for now.
